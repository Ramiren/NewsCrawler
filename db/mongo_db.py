...
def get_article_collection(coll_name):
    global article_collections
    if coll_name not in article_collections:
        if coll_name not in articles_db.collection_names():
...
def group_by_month(coll_name):
    coll = get_article_collection(coll_name)
    oldest = coll.find_one(projection={"_id": 0, "published": 1}, sort=("published", 1))["published"]
...
def get_latest_article_date(coll_name):
    coll = get_article_collection(coll_name)
    return coll.find_one(projection={"_id": 0, "published": 1}, sort=("published", -1))["published"]
